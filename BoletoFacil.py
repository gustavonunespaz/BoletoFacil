...
def process_txt_file(txt_file_path):
    ...
    while i < len(lines):
        ...
        # Verificando se o formato do bairro/cep/cidade/estado está diferente
...
def imprimir_selecionados(event=None):
    ...
    txt_clientes_correios_path = os.path.join(pasta_correios, f"{data_compilacao.strftime('%d-%m-%Y')}.txt")
    try:
